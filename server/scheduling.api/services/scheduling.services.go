...
const schedulingPath = "scheduling"

func SetupRoutes(router *gin.Engine, apiBasePath string) {
	schedulingService := router.Group(fmt.Sprintf("%s/%s/", apiBasePath, schedulingPath))
	schedulingService.GET(fmt.Sprintf("%s", "get_five_day_forecast"), middleware.CheckJWT(), getFiveDayForecast)
}
