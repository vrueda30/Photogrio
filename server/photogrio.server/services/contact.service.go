...
func SetupContactRoutes(router *gin.Engine, apiBasePath string) {
...
func getContact(ctx *gin.Context) {
	...
	contact, cerr := GetContact(contactId, middleware.AccountCookie.AccountId)
	if cerr != nil {
		log.Print(cerr)
...
func getContacts(ctx *gin.Context) {
	...
	if acctId != middleware.AccountCookie.AccountId {
...
func updateContact(ctx *gin.Context) {
	...
	if updatedContact.AddressId == 0 {
		var address = models.Address{Address1: updatedContact.Address.Address1, Address2: updatedContact.Address.Address2, City: updatedContact.Address.City,
			State: updatedContact.Address.State, Zip: updatedContact.Address.Zip, AccountId: middleware.AccountCookie.AccountId}
